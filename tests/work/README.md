...
tests/work/new-run.sh context-compact-soft
# or: python -m tests.work.harness generate context-compact-soft

# 2) Agent step (NOT automated) — see "What you must do" below

# 3) Analyze (deterministic)
python -m tests.work.harness analyze tests/work/runs/context-compact-soft-<timestamp>
```
...
| Step | Tool | Output |
|------|------|--------|
| List scenarios | `python -m tests.work.harness list` | names |
| Generate run | `new-run.sh` / `harness generate` | `tests/work/runs/<scenario>-<stamp>/` git repo + `DRIVE.md` |
| Hard invariant checks | `harness analyze` | `analysis.md`, `results.json`, exit code |
| Harness unit tests | `python -m pytest tests/work/harness/tests -q` | parser/analyze regression |
...
| **Pass (driver)** | Run host_command, then `/work:continue` / execute continue; resume_loads + NEXT |
...
Optional host hooks: @work `references/hooks/reclaim-hooks.md` (Stop coach + SessionStart re-seed).
...
mkdir -p tests/work/history/<run-id>
cp tests/work/runs/<run-id>/analysis.md tests/work/history/<run-id>/
cp tests/work/runs/<run-id>/results.json tests/work/history/<run-id>/
```
