...
7 review sub-sessions and the worker sidechains under `…/subagents/`).
...
- `resolve-conflict-fires`: **satisfied (×2)** — CTM-2 merged clean; CTM-3 conflicted on `cli.py`
  → resolve-conflict `@8cb88fb`; CTM-4 conflicted on 3 regions → resolve-conflict `@ec208a4`
...
- `fix-integration-fires`: **not satisfied** — the CTM-5 plan pre-empted the intended
  empty-store divide-by-zero with an `if total==0` early return (`orchestrator.md:60,63`), so the
  integration test never broke and no fix-integration was dispatched (0 of 17). Benign:
  scenario-seed weakness, not a role regression.
- `review-fix-loop`: **not satisfied** — all 5 review returns are `APPROVED`; zero
  `FIX_REQUESTED` anywhere in the transcript. No implement re-dispatch occurred.
...
- **C2 — Reviewer wrote no decision logs (LOW).** Planner and implement wrote `<role>-N.md` per
...
- **C3 — Stale `log_file` pointers in state.yml (LOW).** For CTM-2/3/4/5, `last_completed.role:
  reviewer` but `last_completed.log_file` points at `…/plan-1.md` (a fallback, since the
  review log does not exist). Same root cause as C2.
- **C4 — Reviewer approved a spec-nonconforming implementation (MEDIUM, real escape).** CTM-4
  ...
  (1-based, like `list`). The CTM-4 review return is bare — unlike the AC-by-AC verification in
...
## Proposals
- `src/parallel/functions/reviewer.md` — **C4** — **applied** — added one scope bullet: a green suite is
  ...
  return.
- `src/parallel/functions/worker-contract.md` — **C1 (worker side) / C2 / C3** — **applied** — workers now
...
- `tests/swarm/harness/ingest.py` — **C1 (harness side)** — **applied** — added a transcript
  ...
  `tool_result` returns. On this run it recovered 18 returns (6 plan / 5 implement / 5
  review / 2 resolve-conflict). Both directions were approved so future runs are covered either
...
clean and correct but exercised only 5 of 7 engineered behaviors; the fix-integration and
