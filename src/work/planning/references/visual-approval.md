...
Optional **human-review surface** for `/work:plan` approval. Turns the draft
...
| `always` | Attempt whenever `/work:plan` reaches the approval gate |
...
**Context-engineering fit** (full norms: @work `references/context-engineering.md`):
...
1. **Load** @work (`planning/templates/visual-plan.html`) as the shell (CSS, layout,
...
- Not a substitute for `/work:plan` task breakdown or DoD.
