...
(`work:claim` / phase table). Record the overlay on the run when it differs. Full tiers:
@work `references/approval-boundaries.md`.

## Issue Creation (from /work:refine, file mode)

When requirements are complete in file mode, `/work:refine` can optionally create a tracking issue.
...
## Issue Update (from /work:refine, PM mode)
...
## Plan-Time Status Update (from /work:plan)

After the user approves an implementation plan, `/work:plan` updates the work item based on the detected PM tool:
...
The decision layer is rewritten in place; the tracker is the realization layer; the cancel/fold *reason* (not a body tombstone) is what preserves provenance in the tracker's state history. See @work (`references/decision-records.md`).
...
- **File mode**: `requirements.md` is created by `/work:refine` and consumed by plan/execute/review. PM issue creation is an optional add-on. Best for ad-hoc work, personal projects, or teams without PM tooling.
- **PM mode**: The PM issue (Linear/Jira) is the source of truth for requirements. `/work:refine` writes requirements back to the issue directly — no `requirements.md` is created. `implementation-plan.md` and `session-state.md` are still created in `./planning/<project>/`. Best for teams where PM issues are the canonical location for requirements.
...
|-------|----------|--------|
| `work:claim` or `work:resume` | yes | claim vs resume |
...
```text
work:claimed unit=LIN-123 channel=terminal phase=needs_plan track=micro
work:await_user unit=LIN-123 gate=plan_approval
work:review method=work-review date=2026-07-18 verdict=clean P1=0 P2=0 P3=0 disposition=none
work:merged local main=@abc1234 compound=done|none — <reason>
work:hard_stop reason=path_not_established
```
...
| Label | Meaning |
|-------|---------|
| `work:claimable` | Eligible for conventions PM queue when NEXT empty |
| `work:blocked` | Do not claim as next work |

**Legacy (accept, prefer `work:`):** claim/resume blocks and labels may still use the
`workflow:` prefix (`workflow:claim`, `workflow:claimable`, …) — same fields/meaning.
