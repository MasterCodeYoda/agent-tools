...
Reusable templates for planning work in **vertical-slice** and **deliverable-partition** decomposition modes. See @work for mode selection criteria.
...
- Mode-appropriate quality gates per @work (`execution/quality-checkpoints.md`)
...
- Use `/work:plan --worktree` to create a worktree at planning time with docs committed, OR manually commit planning docs before running `/work:execute --worktree`
- Stories in the same parallel group must not modify the same files
- Each parallel session uses: `/work:execute --worktree ./planning/<project>/`
...
@work (`references/context-engineering.md`).
...
- [ ] Tests passing (positive + negative cases per @work `execution/quality-checkpoints.md`)
...
Written by `/work:plan` to `./planning/<project>/session-state.md` after plan approval. The generic schema, as it evolves during execution, is in @work › Session Continuity.
...
2. Run `/work:execute ./planning/[project]/` when ready
...
@work (`planning/references/visual-approval.md`).

## Plan Approval Prompt

Presented by `/work:plan` at the approval gate — before any documents are saved, PM tools updated, or execution started.
...
(Executable source of truth for `/work:execute`. The visual plan is presentation only.)
...
Presented by `/work:plan` (Step 7) only when the plan has 2+ independent slices/stories.
...
2. **In a new terminal** (for additional parallel sessions):
   ```bash
   /work:execute --worktree ./planning/[project]/
   ```
...
**Or** use `/work:plan --worktree` next time to automate worktree creation at planning time.
...
When the scenario is ambiguous, default to vertical-slice for feature work and deliverable-partition for foundation, infrastructure, or cross-cutting work. See @work for full mode selection criteria.
