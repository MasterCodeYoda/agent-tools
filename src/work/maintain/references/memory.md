...
**Load when:** `/work:maintain` runs the **memory** job (ritual when due + approved,
`--all`, or a scoped force: `--memory`, `--level`, `--focus`, `--migrate-solutions`).

Primary entry: **`/work:maintain`**. Compat: `/work:compound --maintain` shims here.

# Memory job

Evaluate memory quality across:
...
(prune + line yield + memory hygiene) lives on `/work:maintain`, not on drive
...
Then update `.agent-tools/memory/state.yml` (see also `cadence.md` for stewardship fields):

```yaml
schema_version: 1
interval_days: 7
last_stewardship_at: <ISO-8601>   # set by /work:maintain host when session completes
last_yield_at: <ISO-8601|null>    # leave unchanged in memory-only scoped runs
