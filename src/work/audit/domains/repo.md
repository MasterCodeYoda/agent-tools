...
Consumed by `/work:audit` orchestrator. Use `--focus repo` for this domain only.
...
4. **Re-audit after fixes** — Run `/work:audit --focus repo` to verify progress
5. **Save report** — Export findings to `./planning/repo-audit-report.md`
6. **Run complementary audits** — `/work:audit --focus code`, `/work:audit --focus tests` for code-level findings
```

## Integration Points

### With /work:audit --focus code
...
### With /work:audit --focus frontend
