...
Consumed by `/work:audit` orchestrator. Use `--focus code` for this domain only.
...
| `./tests/` or `./test/` | Test directory | Redirect to `/work:audit --focus tests` |
...
**observability-readiness-analyst** — References @work (`execution/logging.md`):
...
**production-readiness-scout** — References quality checkpoints (`src/work/execution/quality-checkpoints.md`):
...
3. **Re-audit after fixes** — Run `/work:audit --focus code [same scope]` to verify
