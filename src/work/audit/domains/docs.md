...
Consumed by `/work:audit` orchestrator. Use `--focus docs` for this domain only.
...
- API spec drift: if OpenAPI/GraphQL specs exist, verify endpoint descriptions, parameter names, and response schemas match actual handler implementations (cross-domain with `/work:audit --focus api`)
...
3. **Re-audit after fixes** — Run `/work:audit --focus docs [same scope]` to verify
...
### With /work:refine and /work:plan
