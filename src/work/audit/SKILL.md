...
This is `/work:review` at project scope — not targeted at a changeset, but at the full codebase.
...
- **Measure before promoting a claim.** Counts, blast radius, and "this check never fails / never runs / always misses X" are hypotheses until measured. Before stating them as fact in a finding (and especially before Recommended Actions become tickets or plan inputs), reproduce against the current code: run the failing case, count with a command you cite, or check out the pre-fix path and watch the old behaviour. Unmeasured radius stays labeled hypothesis; do not invent precision (exact file counts, "N checks", "always/never") from reading alone.
...
Each domain's agents, skill references, tiered analysis, and check criteria are defined in the @work audit skill under `domains/`. The orchestrator reads these definitions to know what to dispatch.
...
| Documentation | `domains/docs.md` | 2-4 | @work |
...
After deduplication, run the Critic Pass over the merged findings — see @work
...
[If prior audit sessions exist for this project in ~/.claude/history.jsonl — see @work references/conversation-analysis.md]
...
- `/work:audit --focus code` — Full code quality report
- `/work:audit --focus tests` — Full test quality report
- [etc.]
```
...
### With @work audit skill (domain definitions)

Domain definitions live in `domains/` under this skill (`src/work/audit/domains/`). Each file defines the agents, skill references, and check criteria for one domain. The orchestrator reads them; `--focus <domain>` routes to the specific domain definition for focused depth.
