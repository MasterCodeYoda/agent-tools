...
Apply on **every** `/work:continue` orientation (before portfolio mode resolve), and after
...
|-------|-----------------|
| `/work:continue` | **Actionable** — events for the unit machine / portfolio router; may force work on a prior slice before claiming new work |
| bare `/work` (status) | **Advisory only** — same detection checklist; surface in the status report; **never** remediate, claim, or block (status does not claim) |
...
**job class** against @work `references/approval-boundaries.md`:
...
1. Prefer a completed **pre-wake** pass (@work `references/pre-wake-checklist.md`) —
...
findings counts (theater evidence), **surface it** and either re-run `/work:review` +
...
refine/plan loop. Offer process memory capture; do **not** edit work skills in-place.
...
- Full rule: @work `references/approval-boundaries.md` › Claim-class live verification.
...
running @work `references/context-compact.md` (soft path always valid) before more edits.
Full norms: @work `references/context-engineering.md` + `references/context-compact.md`.
...
Load due/signal rules from @work `maintain/references/cadence.md` (read
`.agent-tools/memory/state.yml` + ledger counts).

| Host | Behavior |
|------|----------|
| **`/work:continue`** | On **end-of-loop stop / recap / hard_stop** (not every phase-return): if `offer_due AND signal`, **one** approval prompt — run `/work:maintain` (bare ritual) / snooze 3d / skip once. Never auto-run. Never block claim. Never force `--all`. |
| Bare **`/work`** (status) | Advisory line only when due — do not prompt multi-choice or write state |
...
**Compat:** `/work:continue --yield` still accepted → hand off to
`/work:maintain --yield` (no unit claim).

## Active parallel run

If `.agent-tools/parallel/active-run` is present, portfolio-router rows 2–4 take precedence
over soft-check-driven new claims on parallel-mode-owned items.
