...
**Load when:** `/work:continue` has finished orientation soft-checks and must choose *how*
to drive work — before claiming a single unit into the phase machine. Bare `/work` only
...
@work `parallel/MODE.md` + `orchestrator.md` / `resume.md`.
...
unit to keep the job “busy.” See @work `references/pre-wake-checklist.md` and
...
| 1b | Explicit **multi-item** goal in `$ARGUMENTS` (≥2 issue keys, milestone, backlog file, or peer list) | **parallel** | Enter @work `parallel/orchestrator.md` with that goal (pre-launch confirmation still applies). Require parallel **ready** or one-ask setup. |
| 2 | `.agent-tools/parallel/active-run` present and run is `in_progress` or `terminal_pause` (or equivalent paused status) | **parallel_resume** | Load @work `parallel/resume.md` then re-enter orchestrator. Do not claim a sequential unit from under the run. |
...
| 4 | Same as row 3 but parallel **not** ready (no charter / no `.agent-tools/parallel` setup) | **fallback** | **One** ask: run `/work:setup` then handoff, **or** proceed sequential on the first claimable peer. If sequential, claim one unit only. |
...
3. One-line announce, then load @work `parallel/orchestrator.md` with that goal.
...
- Pasted greppable claim block (see @work `planning/pm-integration.md`):
...
Resume: `work:resume` + `unit:`.

Legacy: `workflow:claim` / `workflow:resume` accepted; prefer `work:`.

## PM queue (row 8b)

Only when conventions document a **closed** filter (e.g. label `work:claimable` + state +
...
Full dialect: `@work:roadmap`. Summary:
