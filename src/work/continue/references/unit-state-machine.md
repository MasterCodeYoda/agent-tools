...
**Also load:** @work `references/tracks.md` (track classification), `references/planning-root.md`
...
3. `requirements.md` or PM issue ACs (file vs PM mode — `@work`)
...
|------|---------------|-----|--------|
| `fuzzy` | direction unchosen | `await_user` or run brainstorm | `/work:brainstorm` (this unit only) |
| `fuzzy` | `DIRECTION_CHOSEN` | `needs_refine` | — |
| `needs_refine` | — | (run) | `/work:refine` (creates shell as needed) |
| `needs_refine` | `REQUIREMENTS_READY` | `needs_plan` | — |
| `needs_plan` | — | (run) | `/work:plan` |
...
| `ready_execute` | — | (run) | `/work:execute` |
...
| `needs_review` | — | (run) | `/work:review` (depth from track; infer, don't ask) |
...
| `needs_compound` | — | (run) | `/work:compound` or record skip |
...
`/work:roadmap`. Do not invent streams inside the unit machine.
