...
- Cold-start / path-not-established from `/work:continue`.
- Resequence after reality changed.

**Not this skill**

- Single fuzzy concept → `/work:brainstorm`
- Exact requirements for one unit → `/work:refine`
- Task breakdown → `/work:plan`
- Driving the loop → `/work:continue`

### Bail if single unit

If charting shows **one** claimable unit and no multi-stream structure, **stop without writing a
map**. Offer `/work:brainstorm` or `/work:refine` instead.
...
- **`/work:continue` is the drive entry** — it **consumes** NEXT / order notation; it does
  **not** author this map. Bare `/work` only **previews** the same path (status).
- Portfolio mode (see `@work:continue`): explicit target → active parallel resume →
