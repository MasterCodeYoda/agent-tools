...
| `./planning/<project>/brainstorm.md`   | Start from a brainstorm seed concept (see `/work:brainstorm`) |
...
independently claimable workstreams, no map), **offer** `/work:roadmap` once and wait — do not
...
**If issue key or PM URL**, fetch issue details using the Issue Retrieval Strategy from @work (PM integration).
...
Determine whether this refinement uses **file mode** or **PM mode**. Follow the detection logic from @work
...
Refinement output shape depends on decomposition mode (selection criteria in @work; full doctrine in @work (`references/decomposition-modes.md`)):
...
rejected alternatives. See @work (`references/decision-records.md`).

## Phase 2.5: Codebase research + technical design (default for feature / hard)

**Load** @work (`references/context-engineering.md`) › On-demand codebase research and
...
**Not brainstorm:** if the *concept* is still fuzzy, stop and offer `/work:brainstorm` first.
...
`blocked_by` / `parallelizable_with`; per-deliverable DoD per @work
(`execution/quality-checkpoints.md`). Full deliverable-partition template: @work
...
(@work `references/critic-pass.md`). Fold survivors into must-haves / open questions; user decides
...
MCP calls and field mappings: @work (`planning/pm-integration.md`).
...
shapes (prefer @work `planning/pm-integration.md` for authoritative field mappings).
