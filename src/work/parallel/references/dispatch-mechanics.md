...
filter to items whose next stage has no upstream blocker:
  refined         → plan can dispatch
  planned         → implement can dispatch
  implemented     → review can dispatch
  fix-requested   → implement can re-dispatch
...
**File-collision avoidance (MVP):** not pre-checked. Two parallel implement dispatches may touch
overlapping files; conflicts surface at merge time and are handled by the resolve-conflict
dispatch.

**Backlog quality:** scheduling produces good parallelism only when items carry
`blocks`/`blocked_by`/`parallelizable_with` (from `/work:refine`). Missing metadata → fall
...
prompt = worker-contract.md content                      (prepended mechanically)
       ...
       + charter load instruction (same-CLI or cross: "Read the project charter files under
         ...
         sections of project.md / engineering.md / workflow.md for your function. Do not assume
...
Pass the per-function **model** (from `config.models`, mapped to a concrete ID). Workers operate
in the item's worktree; entering an existing worktree is done via `cd <worktree-path>` in the
worker prompt.

This ensures charter content is loaded only inside active parallel worker sessions (when needed),
...
|-----------|----------------|
| Create per-item worktree | Planner runs `/work:plan --worktree` → `/git:worktree-create`. Orchestrator never calls `git worktree add`. |
...
| resolve-conflict / fix-integration | Operate on `main`; no worktree. |
...
if conflict:
  ...
  dispatch ONE resolve-conflict (workspace = main, merge in progress)
...
if tests fail:
  capture diagnostics
  dispatch ONE fix-integration (workspace = main, post-merge)
