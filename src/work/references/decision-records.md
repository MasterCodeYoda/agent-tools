...
How decision records and the docs around them are kept across a project's life. The `/work`
skills `@`-reference this file; the per-project bindings (which file plays which role, and the rare
genre override) live in `planning/conventions.md`, authored by `/work:setup`.
...
collapse into one file with two sections; the lifecycle below is identical. `/work:setup` names
...
- **Keep the why, drop the changelog.** Always retain rationale and rejected alternatives — they keep
  ...
  (`.agent-tools/memory/entries/`, type `lesson`) via `/work:compound` — not into the
...
1. **At authoring** (`/work:brainstorm`, `:refine`): validate the decision against the live corpus
...
2. **At close** (`/work:execute`, folded into "verify before closing"): reconcile the governing
