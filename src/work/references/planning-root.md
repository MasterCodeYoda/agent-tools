...
**Load when:** any `/work:*` phase reads or writes planning artifacts, conventions,
...
exists, **create** `.agent-tools/planning/` (via `/work:setup` or first phase that needs
...
| `.agent-tools/runs/` | Run event spine + closed-run ledger (see `runs-ledger.md`); yield via `/work:maintain` |
| `.agent-tools/charter/`, `.agent-tools/parallel/` | Charter + parallel-mode setup |

## Migration (legacy `./planning/`)

**Owner:** `/work:setup` only (initialize and maintain). Full procedure with cases A–D,
...
`/work:setup` authors hygiene under the **active** planning root after migration resolve.
