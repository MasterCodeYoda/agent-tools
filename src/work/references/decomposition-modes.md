...
Full doctrine for the two decomposition modes summarized in the parent @work skill — when and how to use each mode, why each works where it does, verbatim AC inheritance and backlog resize, implementation order within a mode, and the anti-patterns to avoid.
...
The selection criteria live in the parent skill — see @work (SKILL.md § Mode Selection).
