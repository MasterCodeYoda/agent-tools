# Work status (bare `/work`)

**Load when:** user invokes bare **`/work`** (family parent, no drive intent). This is the
**orientation entry** — portfolio glance only. Drive work with **`/work:continue`**.

Summarize → stop. Never claim, phase-invoke, ledger-append, merge, or
auto-parallel from status.
...
| Unknown junk | Full status + one line: drive with `/work:continue …` |

Do **not** treat bare + target as drive. Drive always says `/work:continue`.
...
- Enter parallel orchestrator or parallel resume
...
- Read `.agent-tools/parallel/active-run` + light `state.yml` summary when present
...
- **Surface** soft-check signals as advisory lines (same checklist as continue
  ...
  `/work:maintain` (never run maintain from status)
- **Preview** which portfolio mode `/work:continue` would enter (mode name + target) without entering it
...
3. **Parallel run** — if `.agent-tools/parallel/active-run` present, summarize run id / status / item
   stages lightly. Do not resume (drive with `/work:continue`).
...
6. **Soft signals** — apply continue soft-check *detection* only (theater review evidence,
   ...
   @work `maintain/references/cadence.md`). List; do not fix.
...
**Suggested:** `/work:setup` to scaffold planning hygiene + optional conventions.
```
...
**Stewardship:** ok | due — `/work:maintain` (<signal one-liner or last date>)
**Continue would:** `<mode>` → <target or hard_stop reason>

**Drive:** `/work:continue` · focused: `/work:continue <id|path|slug>`
**Steward:** `/work:maintain` · **Other:** `/work:roadmap` · `/work:brainstorm` · parallel mode · `/work:setup`
```
...
| | `/work` (status) | `/work:continue` (drive) |
|--|----------------------|------------------------------|
