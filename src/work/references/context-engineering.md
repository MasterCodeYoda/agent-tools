...
See @work (`continue/references/unit-state-machine.md`) for events and transitions.
...
`/work` exists so correct behavior is the **default** without magic phrases. That only
...
**Not** `/work:brainstorm` (concept: *what/why* seed). Brainstorm stays optional and
...
Template support: @work (`planning/templates.md`) › Implementation Plan — **Research
...
Writing IC is not enough when **work remains**. **Load and run** @work
...
still `implementation-plan.md` (@work `planning/references/visual-approval.md`).
