...
**Load when:** packaging `/work` + parallel mode for another agent runtime (Hermes, custom
...
|----------|------------------|--------------------|
| Status entry | bare `/work` | Read-only portfolio status (`references/status.md`); no claim / no drive |
| Drive entry | `/work:continue` | Orient → portfolio mode → unit SM or swarm; **never invent NEXT** |
...
| Stewardship | `/work:maintain` | Prune check + yield + memory hygiene; cadence; no skill mutation |
