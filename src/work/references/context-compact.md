...
session), then **continue the same workstream** via `/work:continue` or execute continue
on that unit — without replaying tool noise.

Project-agnostic — planning-root / unit paths only (@work `references/planning-root.md`).

**Related norms:** dumb-zone bands and IC *content* fields —
@work `references/context-engineering.md`. This file owns **control flow**.
...
| `continue` | `work:continue` or `workflow:execute continue` |
...
3. **Continue same unit** — `/work:continue` or execute continue; no portfolio rediscovery
   unless unclaimed.  
...
`/work:continue` (or `/work:execute continue`) — read resume_loads, restate NEXT.
...
| Execute / continue | `@work:execute`, `@work:continue` |
