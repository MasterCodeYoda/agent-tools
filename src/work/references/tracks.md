...
**Load when:** unit mode classifies a claimed unit, or `/work:setup` authors track
...
No track → hard-stop + offer `/work:roadmap`. Do not stuff a program into micro or research.
...
**Invoke:** `@work:execute` direct-issue path (`execution/references/direct-issue-execution.md`)
...
`/work:review` **does not ask** depth by default — resolve from track → flags → diff size
