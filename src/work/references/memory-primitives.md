...
Reference for the file types, settings, hooks, and slash commands that govern what an agent remembers across sessions. Used by `/work:compound` (capture) and `/work:maintain` (stewardship: prune + memory hygiene + yield cadence); consulted when memory feels stale, bloated, or mis-scoped.
...
**L3-shared layout** (authored by `/work:setup` + `/work:compound`; stewarded by `/work:maintain`):
...
sections Rule + Mandatory verify + Load when (`/work:compound` → `templates/dated-rule.md`).
...
**Loading:** `AGENTS.md` carries a marker-bounded pointer to `MEMORY.md` (installed by `/work:setup`). Agents read the index on demand (compound, unfamiliar seams, debugging); they do **not** `@`-import every satellite every turn.
...
Team-shared technical memory for agent-tools projects is **`.agent-tools/memory/`**. Prefer writing project-wide captures there via `/work:compound`; steward quality via `/work:maintain`; keep Claude auto-memory for personal/machine notes or thin pointers.
...
Installed by `/work:setup`; capture via `/work:compound`; steward via `/work:maintain`:
...
- **Capture routing:** deterministic gate in `/work:compound` (solutions → `solutions/`; project-wide patterns/gotchas → `entries/`; decisions → ADRs; voice → personify)
- **Maintain:** `/work:maintain` ritual prune-checks `planning/`, audits L1–L3 (+ run yield when due), proposes promote/retire, optional `--migrate-solutions` for legacy `docs/solutions/` (compat: `/work:compound --maintain`)
- **Cadence:** `state.yml` `interval_days` (default 7) — see @work `maintain/references/cadence.md`; soft-prompted from status/continue/capture; `:maintain` always on-demand
...
| `/work:maintain`                   | Command (this repo) | Stewardship: planning prune check + run yield + three-tier memory audit (promote/retire; optional `--migrate-solutions`). Compat: `/work:compound --maintain`. |
...
**Claude example**: Use `/memory` to review and delete entries, or `/work:maintain --focus staleness`.
...
- Maintain due per `.agent-tools/memory/state.yml` (`interval_days`, default 7) — soft-prompted from status/continue/capture; run `/work:maintain` anytime on demand
...
- Legacy `docs/solutions/` still exists and should be migrated (`/work:maintain --migrate-solutions`)
- Run yield is stale or ≥5 closed runs since last yield glance

The specific triggers and tools vary by agent platform. Primary tool: **`/work:maintain`**.
