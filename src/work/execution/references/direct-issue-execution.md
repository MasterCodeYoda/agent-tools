...
Load from `/work:execute` **or** from `/work:continue` when unit track is **micro**
(@work `references/tracks.md`). This is a first-class track process, not only an execute
side door.
...
```
1. Fetch issue (or use paste) via Issue Retrieval Strategy — @work PM integration
2. Extract title, description, ACs if present
3. Light on-demand codebase research (default) — @work references/context-engineering.md
4. Confirm scope with user (issue confirmation = plan approval on micro)
5. Ensure working branch ≠ main (branch naming @work)
...
11. PM Done + work:merged egress line when PM mode
```
