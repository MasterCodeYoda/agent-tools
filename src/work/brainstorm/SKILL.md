...
Turn **one** loose, fuzzy idea into a crisp, framed concept that `/work:refine` can take into
...
**Skip it** and go straight to `/work:refine` when you can already state the problem and a
...
- Concept still fuzzy at program scale → user should run `/work:brainstorm` only if they first
  narrow to **one** concept; otherwise go to `/work:roadmap` (roadmap owns destination framing
  for multi-unit horizons).
- Direction clear enough to name streams/order → `/work:roadmap`.
...
| **Forbidden until user converges** | Writing `Chosen Direction`; `Status: Explored`; offering `/work:refine` as done; collapsing to one option; same-turn recommendation |
...
5. **Pressure-test (optional).** If the chosen direction is a big or hard-to-reverse bet, run
   **Pass 1 (Blind-Spot)** of the Critic Pass — see @work (`references/critic-pass.md`).
...
`/work:refine` uses), create `./planning/<project>/` if needed.
...
This artifact is local-file only. PM-system framing begins at `/work:refine`, not here.

## Handoff to Refine

Only after `Status: Explored`: "Concept framed and saved to `planning/<project>/brainstorm.md`.
Run `/work:refine` to turn this seed into structured requirements."

## Key Principles

- **Diverge before you converge** — and **stop** so the user actually converges.
- **Single-concept only** — multi-unit horizons belong to `/work:roadmap`.
