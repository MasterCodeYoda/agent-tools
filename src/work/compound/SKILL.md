---
name: work:compound
description: Capture durable knowledge from any engineering work — debugging solutions, refactors, features, design decisions, reusable patterns — so each unit of work compounds the next. Stewardship (prune + yield + memory) is /work:maintain; --maintain is a compat shim.
argument-hint: "[context about what was solved] | --maintain → /work:maintain (compat)"
user-invocable: true
---
...
**`/work:maintain`**. Do not conflate capture with scaffolding hygiene.
...
|---|---|---|
| `--maintain` or any `--maintain --…` flags (`--level`, `--focus`, `--migrate-solutions`) | **Compat → maintain** | Run **`/work:maintain`** with the same flags (memory-scoped). Prefer telling the user the primary command is `:maintain`. |
...
*capture* invocations only soft-point at `/work:maintain` — see [Maintain Due Check](#maintain-due-check-capture-and-bare-compound).
...
If this tree is missing, run `/work:setup` (or create it during capture/maintain with user confirmation). Do **not** write product docs under `docs/solutions/` for new captures.

**Non-goals for this tree:** ADRs, CONTRIBUTING/gate matrices, Codex/domain docs, planning scratch, personify voice, secrets.

**Primitives reference:** @work (`references/memory-primitives.md`).
...
Light check only: skim AGENTS.md, CONTRIBUTING.md (if present), `docs/decisions/` titles/index, existing `entries/`. Deep doc consistency is `/work:audit`.
...
```bash
/work:compound "Fixed N+1 query in brief generation"
```
...
### 3. With Context Hint
```bash
/work:compound "resolved circular dependency in auth module"
```

## Maintain Due Check (capture and bare compound)

Before capture work (not when args already shim to `/work:maintain`):

1. Load due rules from @work `maintain/references/cadence.md` (or read
   `.agent-tools/memory/state.yml` with the same formula).
2. If **due**: soft-prompt stewardship — offer **`/work:maintain`** now / snooze 3d /
...
`references/undocumented-surfacing.md` (uses @work `references/conversation-analysis.md`).
...
skill paths (portable seed fields; see @work `references/runs-ledger.md`). Do not patch
skills here. Line yield and cadence live on **`/work:maintain`**.

### With docs promote

Stable architecture/product entries may promote to `docs/` (Step 5). `/work:maintain`
(memory job) may flag promote candidates (see @work `maintain/references/memory.md`).
...
Hand off to **`/work:maintain`** with the same flags — load `@work:maintain` SKILL.md
