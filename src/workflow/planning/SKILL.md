...
Detect file vs PM per @workflow `planning/pm-integration.md` (explicit path/key → artifacts →
project context → MCP → fallback file). State determination; allow course-correct.

## Decomposition mode

Doctrine: @workflow `references/decomposition-modes.md`.
...
@workflow `planning/templates.md` Variant A/B.
...
Task breakdown patterns: `planning/task-breakdown.md`. Density/segmentation: @workflow
