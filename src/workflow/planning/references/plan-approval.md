...
**Load when:** `/workflow:plan` reaches the approval gate (after draft + leverage check +
...
Show the plan summary using the Plan Approval Prompt from @workflow (`planning/templates.md`).
...
4. Rename branch per @workflow `references/family-contracts.md`  
5. Establish dependencies via @workflow dependency-establishment  
...
When 2+ independent slices: prompts from @workflow (`planning/templates.md`) › Parallel
Execution Prompts.

**Approve & Save** — stop; remind user how to run execute (with/without worktree).  
**Approve & Execute** — handoff to `/workflow:execute ./planning/[project]/`.
